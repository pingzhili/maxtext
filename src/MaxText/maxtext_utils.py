...
def calculate_indexer_tflops_per_device(config):
  ...
  k_flops = 2 * batch_len * config.emb_dim * (config.index_head_dim + config.index_n_heads)
...
def calculate_mla_tflops_per_device(config):
  ...
  if config.use_sparse_indexer:
    ...
    attention_flops = 2 * batch_len * effective_seq * config.num_query_heads * (qk_head_dim_sum + config.v_head_dim)
